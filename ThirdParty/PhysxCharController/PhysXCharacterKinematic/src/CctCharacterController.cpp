...
/*		if(direction.y==0.0f)
		...
		if (collide_only) HasMoved = false;
		else HasMoved = true;
		...
		else
		{
			...
			if (collide_only)
			{
				...
				if(touchedShape)
				{
					...
					if(attachedToActor.getType() == PxActorType::eRIGID_DYNAMIC)
					{
						HasMoved = true;
					}
				}
...
//			WorldNormal.normalize();
	...
	if (! collide_only) swept_volume.mCenter = CurrentPosition;
...
//	printf("StepOffset: %f\n", StepOffset);
	...
	if(true)
	{
		...
		if(doSweepTest(userData, userHitData, userObstacles, volume, SideVector, MaxIterSides, &NbCollisions, min_dist, filters, false, true))
		{
			if(NbCollisions)
